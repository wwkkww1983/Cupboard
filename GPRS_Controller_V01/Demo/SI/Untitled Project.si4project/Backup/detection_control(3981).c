...
char Lock_Control_Thread( thread_t* pt )
{
    ...
    while( true )
    {
        ...
        M26_Gprs.send_data_buf[cnt++] = 0x80; //type
...
void RunDataUpDate( void )
{
    ...
    */
    ...
    if(M26_Gprs.send_data_flag == 0)
    {
        ...
        memset( M26_Gprs.send_data_buf, 0, sizeof( M26_Gprs.send_data_buf ) );
        //359759002514931
        M26_Gprs.send_data_len=sprintf(&M26_Gprs.send_data_buf, "info@@$$@@%s@@$$@@%d@@$$@@%d\r\n",Control.IMEI_Buffer,Control.BatteryCapacity,Control.WorkMode);
...
char Time_100ms_Thread( thread_t* pt )
{
    ...
    while( true )
    {
        ...
        Lock_State_Check();//锁状态检测 开锁=使用 关锁=空闲
        ...
        if( ( Control.Lock_OnOff == 0 ) && ( Control.WorkMode == 0 ) )
        {
